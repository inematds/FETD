#!/usr/bin/env python3
"""
Conversor de Markdown para HTML Rico
Converte os arquivos .md expandidos para HTML com estilo visual Tailwind CSS
Mantém o padrão rico usado nas trilhas existentes
"""
...
def converter_markdown_para_html_corpo(conteudo_md, trilha):
    """Converte o corpo do Markdown para HTML rico"""
...
def processar_secao(secao, cor):
    ...
    if titulo_match:
        ...
        # Remove emojis especiais do título
        titulo_limpo = re.sub(r'^[🎯💡📊🔍✅❌⚡🚀📝🎨🔧💰📈🎓💼🌟]+\s*', '', titulo)

        html_parts.append(f'''
      <!-- Seção: {titulo_limpo} -->
      <div class="mb-16">
        <h2 class="text-3xl font-bold mb-8 pb-4 border-b-2 border-{cor}/20">{titulo}</h2>
''')
...
def processar_subsecao(subsecao, cor):
    """Processa uma subseção (###) individual"""
    html_parts = []
    ...
    if titulo_match:
        ...
        <div class="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 p-6 rounded-lg mb-8">
          <h3 class="text-xl font-bold text-red-700 dark:text-red-400 mb-4">{titulo}</h3>
''')
        elif e_verdade:
            html_parts.append(f'''
        <div class="bg-green-50 dark:bg-green-900/20 border-l-4 border-green-500 p-6 rounded-lg mb-8">
          <h3 class="text-xl font-bold text-green-700 dark:text-green-400 mb-4">{titulo}</h3>
''')
        elif e_exemplo:
            html_parts.append(f'''
        <div class="bg-{cor}/10 dark:bg-{cor}/20 border-2 border-{cor}/30 p-6 rounded-xl mb-8">
          <h3 class="text-xl font-bold text-{cor} mb-4">{titulo}</h3>
''')
        else:
            html_parts.append(f'''
        <div class="mb-8">
...
def processar_tabela(tabela_md, cor):
    """Converte tabela Markdown para HTML com estilo"""
    ...
    html = f'''
          <div class="overflow-x-auto mb-6">
            <table class="w-full border-collapse">
              <thead>
                <tr class="bg-{cor}/20 dark:bg-{cor}/30">
'''

    for col in colunas_header:
        col_formatado = aplicar_formatacao_inline(col)
        html += f'                  <th class="px-4 py-3 text-left font-semibold border border-{cor}/30">{col_formatado}</th>\n'
...
'''
    ...
    for i, linha in enumerate(linhas_dados):
        ...
        for col in colunas:
            col_formatado = aplicar_formatacao_inline(col)
            html += f'                  <td class="px-4 py-3 border border-neutral-200 dark:border-neutral-700">{col_formatado}</td>\n'
        html += '                </tr>\n'
...
def processar_codigo(codigo_md, cor):
    ...
    return f'''
          <div class="bg-neutral-900 dark:bg-black p-6 rounded-lg overflow-x-auto mb-6">
            <pre class="text-sm text-green-400"><code>{codigo}</code></pre>
          </div>
'''

def processar_lista_nao_ordenada(lista_md, cor):
    """Converte lista não ordenada para HTML"""
    ...
    for linha in linhas:
        ...
        if icone_match:
            ...
            html += f'            <li class="flex items-start gap-3"><span class="text-2xl">{icone}</span><span class="flex-1 pt-1">{texto_formatado}</span></li>\n'
        else:
            texto_formatado = aplicar_formatacao_inline(texto)
            html += f'            <li class="flex items-start gap-3"><span class="text-{cor}">•</span><span class="flex-1">{texto_formatado}</span></li>\n'
...
def processar_lista_ordenada(lista_md, cor):
    ...
    html = '          <ol class="space-y-3 mb-6 list-decimal list-inside">\n'

    for linha in linhas:
        # Remove número
        texto = re.sub(r'^\d+\.\s+', '', linha)
        texto_formatado = aplicar_formatacao_inline(texto)
        html += f'            <li class="text-neutral-700 dark:text-neutral-300">{texto_formatado}</li>\n'
...
def processar_definicao(def_md, cor):
    ...
    if match:
        ...
        return f'''
          <div class="mb-4">
            <p class="text-lg"><span class="font-bold text-{cor}">{label}:</span> {conteudo_formatado}</p>
          </div>
'''
    else:
...
def processar_paragrafo(para_md, cor):
    ...
    return f'          <p class="text-lg text-neutral-700 dark:text-neutral-300 mb-6">{texto_formatado}</p>\n'
...
def aplicar_formatacao_inline(texto):
    ...
    # Código inline
    texto = re.sub(r'`(.+?)`', r'<code class="px-2 py-1 bg-neutral-200 dark:bg-neutral-700 rounded">\1</code>', texto)

    # Links
    texto = re.sub(r'\[(.+?)\]\((.+?)\)', r'<a href="\2" class="text-blue-600 dark:text-blue-400 hover:underline">\1</a>', texto)

    return texto

def gerar_html_completo(trilha, modulo_num, titulo, duracao, objetivo, corpo_html):
    """Gera o HTML completo do módulo"""
...
<body class="preload bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        <div class="flex items-center">
          <a href="../../index.html" class="text-2xl font-bold bg-gradient-to-r from-{cor} to-{cor} bg-clip-text text-transparent">
            🎓 FETD
          </a>
        </div>
        <div class="hidden md:flex items-center space-x-8">
          <a href="../../index.html" class="text-neutral-700 dark:text-neutral-300 hover:text-{cor} transition-colors">Início</a>
          <a href="../../trilhas/{trilha}.html" class="text-neutral-700 dark:text-neutral-300 hover:text-{cor} transition-colors">Trilha {nome_trilha}</a>
  ...
  <div class="bg-white dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
      <nav class="flex text-sm" aria-label="Breadcrumb">
        <a href="../../index.html" class="text-{cor} hover:opacity-80">Início</a>
        <span class="mx-2 text-neutral-400">/</span>
        <a href="../../trilhas/{trilha}.html" class="text-{cor} hover:opacity-80">Trilha {nome_trilha}</a>
        <span class="mx-2 text-neutral-400">/</span>
  ...
  <!-- Hero Módulo -->
  <section class="bg-gradient-to-r from-{cor} to-{cor} py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-white">

      <div class="flex items-center gap-3 mb-4">
        <span class="px-3 py-1 bg-white/20 rounded-full text-sm font-semibold">Módulo {modulo_num} de 10</span>
        <span class="px-3 py-1 bg-white/20 rounded-full text-sm font-semibold">⏱️ {duracao}</span>
      </div>
  ...
  <section class="py-16 bg-white dark:bg-neutral-800">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex justify-end mb-8">
        <a href="../../downloads/{trilha}/modulo-{modulo_num:02d}.md" download class="px-6 py-3 bg-{cor} text-white rounded-lg font-semibold hover:opacity-90 inline-flex items-center gap-2">
          <span>📥</span>
...
{corpo_html}
  ...
  <!-- Footer / Navegação -->
  <section class="py-12 bg-neutral-100 dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-700">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center">
        <a href="../../trilhas/{trilha}.html" class="px-6 py-3 bg-{cor} text-white rounded-lg font-semibold hover:opacity-90">
          ← Voltar para Trilha
        </a>
        {f'<a href="modulo-{modulo_num+1:02d}.html" class="px-6 py-3 bg-{cor} text-white rounded-lg font-semibold hover:opacity-90">Próximo Módulo →</a>' if modulo_num < 10 else ''}
      </div>
    </div>
  </section>
...
def converter_modulo(trilha, modulo_num):
    """Converte um módulo específico de MD para HTML"""
    # Caminhos
    ...
    # Converte corpo
    corpo_html = converter_markdown_para_html_corpo(conteudo_md, trilha)
...
def converter_trilha_completa(trilha):
    ...
    for i in range(1, 11):
        if converter_modulo(trilha, i):
            sucesso += 1
    print(f"✅ {sucesso}/10 módulos convertidos\n")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Uso: python converter_md_para_html_rico.py <trilha> [modulo_num]")
        print("Exemplos:")
        print("  python converter_md_para_html_rico.py talento        # Converte todos os 10 módulos")
        print("  python converter_md_para_html_rico.py talento 1      # Converte apenas módulo 1")
        print("")
        print("Trilhas disponíveis: operacional, comunicador, talento, tecnico")
        return
